 ...
 *
 *   # list all todos
 *   curl -s -H "Accept: application/json" http://localhost:8080/todos
 *
 *   # Create new todo
 *   curl -s -X POST -H "Content-type: application/json" -d '{ "title":"Test TODO", "completed":false }' http://localhost:8080/todos
 *
 *   # show todo with id 1
 *   curl -s -H "Accept: application/json" http://localhost:8080/todos/1
 *
 ...
 *   curl -s -X PUT -H "Content-type: application/json" -d '{ "title":"Task 3 Update", "completed":true }' http://localhost:8080/todos/3
 *
 *   # update todo with id 3 (change title) - only the title changed!
 *   curl -s -X PATCH -H "Content-type: application/json" -d '{ "title": "Title via Patch" }' http://localhost:8080/todos/3
 * }
 * </pre>
 */
@RestController
